...
module IKAOPLL_reg #(parameter FULLY_SYNCHRONOUS = 1, parameter ALTPATCH_CONFIG_MODE = 0, parameter INSTROM_STYLE = 0) (
    ...
    //internal clock
    input   wire            i_phi1_PCEN_n, //positive edge clock enable for emulationFV
	...
	// Observation outputs for TB/wrapper (added)
    output wire            o_D9REG_WRDATA_QUEUED_N,
    output wire            o_D9REG_ADDR_MATCH,
	// New: expose core write-request strobes so wrapper can follow core timing
...
// --- (add these regs near the other reg declarations for D9REG, at module scope) ---
`ifdef TB_DEBUG_DISPLAY
reg trace_val;
reg prod_term;
reg rhs_val;
`endif
...
always @(posedge emuclk) begin
	if(!phi1ncen_n) begin
		if(addrreg_wrrq && dbus_inlatch[7:6] == 2'b00) begin
			d9reg_addr <= dbus_inlatch[5:0];
`ifdef TB_DEBUG_DISPLAY
            // print the latched-to value — use dbus_inlatch because <= is non-blocking
            $display("[DBG_ADDR] %0t PHI1 ADDR_WRRQ latched_to=0x%02x dbus_inlatch=0x%02x d9reg_en=%b addrcntr=%0d",
                     $time, dbus_inlatch[5:0], dbus_inlatch, d9reg_en, d9reg_addrcntr);
`endif
		end
	end
end
...
// --- (insert a single debug always block after the d9reg_wrdata_queued_n update) ---
// --- Debug instrumentation (replace the earlier reg declarations + debug always block) ---
`ifdef TB_DEBUG_DISPLAY
// compute terms combinationally so we don't use blocking assignments in a sequential block
wire trace_val_w = trace_d9reg_addrcntr;                // ~|{~i_RST_n, addrreg_wrrq, d9reg_wrdata_queued_n}
wire prod_term_w  = (d9reg_en && datareg_wrrq);
wire rhs_val_w    = (trace_val_w || prod_term_w);

always @(posedge emuclk) begin
    if (!i_RST_n) begin
        // no-op on reset (kept for symmetry)
    end
    else if (!phi1ncen_n) begin
        // Print when addr or data wrrq events occur
        if (addrreg_wrrq) begin
            $display("[CORE_DBG] %0t PHI1 ADDR_WRRQ: dbus_inlatch=0x%02x d9reg_addr=0x%02x d9reg_en=%b addrcntr=%0d addr_match=%b queued_n=%b",
                     $time, dbus_inlatch, d9reg_addr, d9reg_en, d9reg_addrcntr, d9reg_addr_match, d9reg_wrdata_queued_n);
        end
        if (datareg_wrrq) begin
            $display("[CORE_DBG] %0t PHI1 DATA_WRRQ: dbus_inlatch=0x%02x d9reg_en=%b d9reg_data=0x%02x queued_n=%b",
                     $time, dbus_inlatch, d9reg_en, d9reg_data, d9reg_wrdata_queued_n);
        end

        // Evaluate queue expression and snapshot (use combinational wires calculated above)
        $display("[CORE_DBG2] %0t PHI1 QUEUE_EVAL: addrreg_wrrq=%b datareg_wrrq=%b d9reg_en=%b prod=%b trace=%b RHS=%b queued_before=%b addrcntr=%0d d9reg_addr=0x%02x addr_match=%b",
                 $time, addrreg_wrrq, datareg_wrrq, d9reg_en, prod_term_w, trace_val_w, rhs_val_w, d9reg_wrdata_queued_n, d9reg_addrcntr, d9reg_addr, d9reg_addr_match);

        // Print low-nibble match check for easy inspection
        $display("[CORE_DBG3] %0t PHI1 MATCHCHK: addrcntr_lo=%0d addr_lo=0x%0x trace=%b addr_match=%b",
                 $time, d9reg_addrcntr[3:0], d9reg_addr[3:0], trace_val_w, d9reg_addr_match);
    end
end

// --- New: compact monitor that prints only on change to reduce log volume ---
// Keep persistent previous-snapshot registers to compare and print when things change
reg [5:0] dbg_prev_d9reg_addr;
reg [4:0] dbg_prev_addrcntr;
reg dbg_prev_queued;
reg dbg_prev_addrmatch;
reg dbg_prev_addrwrrq;
reg dbg_prev_datawrrq;
reg [7:0] dbg_prev_d9data;
reg dbg_prev_d9en;

initial begin
    dbg_prev_d9reg_addr = 6'h00;
    dbg_prev_addrcntr = 5'd0;
    dbg_prev_queued = 1'b1;
    dbg_prev_addrmatch = 1'b0;
    dbg_prev_addrwrrq = 1'b0;
    dbg_prev_datawrrq = 1'b0;
    dbg_prev_d9data = 8'h00;
    dbg_prev_d9en = 1'b0;
end

always @(posedge emuclk) if(!phi1ncen_n) begin
    // detect any change among the interesting signals
    if (d9reg_addr != dbg_prev_d9reg_addr ||
        d9reg_addrcntr != dbg_prev_addrcntr ||
        d9reg_wrdata_queued_n != dbg_prev_queued ||
        d9reg_addr_match != dbg_prev_addrmatch ||
        addrreg_wrrq != dbg_prev_addrwrrq ||
        datareg_wrrq != dbg_prev_datawrrq ||
        d9reg_data != dbg_prev_d9data ||
        d9reg_en != dbg_prev_d9en) begin

        $display("[MON_D9] %0t ADDR=0x%02x CNTR=%0d ADDR_MATCH=%b QUEUED_N=%b ADDR_WRRQ=%b DATA_WRRQ=%b D9DATA=0x%02x EN=%b",
                 $time, d9reg_addr, d9reg_addrcntr, d9reg_addr_match, d9reg_wrdata_queued_n,
                 addrreg_wrrq, datareg_wrrq, d9reg_data, d9reg_en);

        // update previous snapshot
        dbg_prev_d9reg_addr <= d9reg_addr;
        dbg_prev_addrcntr <= d9reg_addrcntr;
        dbg_prev_queued <= d9reg_wrdata_queued_n;
        dbg_prev_addrmatch <= d9reg_addr_match;
        dbg_prev_addrwrrq <= addrreg_wrrq;
        dbg_prev_datawrrq <= datareg_wrrq;
        dbg_prev_d9data <= d9reg_data;
        dbg_prev_d9en <= d9reg_en;
    end
end
...
assign o_D9REG_WRDATA_QUEUED_N = d9reg_wrdata_queued_n;
assign o_D9REG_ADDR_MATCH      = d9reg_addr_match;
assign o_ADDRREG_WRRQ          = addrreg_wrrq;
assign o_DATAREG_WRRQ          = datareg_wrrq;
