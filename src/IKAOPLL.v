...
module IKAOPLL #(
    ...
    // Test-only busy/done (propagate from IKAOPLL_reg when built with TB_BUS_HANDSHAKE)
    output  wire                o_BUSY,
    output  wire                o_WRITE_DONE,

    // Expose low-level reg/core handshake signals to testbench wrapper
...
//////  REGISTER
////
// observation pad for wrapper (tie-off)
IKAOPLL_reg_wrapper  #(.FULLY_SYNCHRONOUS(FULLY_SYNCHRONOUS), .ALTPATCH_CONFIG_MODE(ALTPATCH_CONFIG_MODE), .INSTROM_STYLE(0)) u_REG (
    .i_EMUCLK                   (emuclk                     ),
    .i_phiM_PCEN_n              (i_phiM_PCEN_n              ),
    ...
    // pass through busy/done (no harm if TB_BUS_HANDSHAKE is not defined in reg)
	.o_BUSY                     (o_BUSY                     ),
